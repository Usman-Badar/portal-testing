...
export const TotalCostCalculation = () => {
    ...
    if (sub_total_value !== null && ac_total !== null && rows !== null) {
        ...
        let sub_total = sub_total_value === '' ? 0 : sub_total_value;
        const calc = (parseFloat(parseFloat(sub_total) + parseFloat(total_calculated_amount)).toFixed(2)) - parseFloat(ac_total).toFixed(2);
    
        document.getElementById('total_calculated_amount').textContent = calc.toFixed(2);
...
export const POSubmittion = ( e, history, Bills, Data, PR, Vendor, specifications, totalACAdjustments, attachedAC ) => {
    ...
    FormFields.append( "totalACAdjustments", totalACAdjustments);
        ...
        res => {
            ...
            if ( res.data === 'success' )
            {
                ...
                setTimeout(() => {
                    ...
                    history.replace('/purchase/order/requests');
                }, 2000);
                JSAlert.alert('Purchase Order Has Been Generated').dismissIn(1000 * 2);
            }else
            {
                $('fieldset').prop('disabled', false);
                JSAlert.alert("Something went wrong, cannot create purchase order!!!").dismissIn(1000 * 2);
            }
    ...
    .then(
        res => {
            ...
            if ( res.data === 'success' )
            {
                ...
                setTimeout(() => {
                    $('fieldset').prop('disabled', false);
                    history.replace('/purchase/order/details?po_id=' + window.location.href.split('&&po_id=').pop());
                }, 2000);
                JSAlert.alert('Purchase Order Has Been Updated').dismissIn(1000 * 2);
            }
...
export const updatePaymentStatus = ( e, po_id, requested_by, setPaymentStatusModel, setRequestDetails, setSpecifications, setAttachedBills, setAdditionalRows, setPRequestDetails, setPRSpecifications, setBills, setPR, setPRCode, setSPRSpecifications ) => {
    ...
    $('fieldset').prop('disabled', true);
    axios.post(
        '/purchase/order/update_payment_status',
        {
            po_id: po_id,
            emp_id: localStorage.getItem('EmpID'),
    ...
    .then(
        res => 
        {
            ...
            if ( res.data === 'success' )
            {
                ...
                JSAlert.alert("The payment status has been updated to " + payment_status);
            }
...
export const releasePayment = ( po_id, requested_by, setRequestDetails, setSpecifications, setAttachedBills, setAdditionalRows, setPRequestDetails, setPRSpecifications, setBills, setPR, setPRCode, setSPRSpecifications ) => {
    ...
    axios.post(
        '/purchase/order/update_cash_status/without_ac',
        {
            po_id: po_id,
            requested_by: requested_by,
            emp_id: localStorage.getItem('EmpID')
...
export const clearSelectedAC = ( po_id, selectedAC, setRequestDetails, setSpecifications, setAttachedBills, setAdditionalRows, setPRequestDetails, setPRSpecifications, setBills, setPR, setPRCode, setSPRSpecifications ) => {
    ...
    $('#clearBtn').prop('disabled', true);
    axios.post(
        '/purchase/order/update_cash_status',
        {
            list: list,
            emp_id: localStorage.getItem('EmpID')
        }
...
export const loadRequests = ( cashier, location_code, companies, CompanyViewer, Admin, setRequests ) => {

    axios.post(
        '/purchase/order/load/requests',
        {
            emp_id: localStorage.getItem('EmpID'),
            accessKey: Admin ? 1 : 0,
